...
class AdvRunner:
    def __init__(self, model, attack, l0_hist_limits, data_RGB_size, device, dtype, verbose=False):
        ...
        self.data_RGB_size = data_RGB_size
    ...
    def run_standard_evaluation(self, x_orig, y_orig, n_examples, bs=250):
        ...
        with torch.cuda.device(self.device):
            ...
            for batch_idx in trange(n_batches):
                ...
                batch_l0_norms_adv_perts, batch_l0_norms, all_batch_succ, all_batch_loss = self.attack.perturb(x, y)
                ...
                with torch.no_grad():
                    ...
                    batch_l0_norms = batch_l0_norms_adv_perts.abs().view(bs, 3, -1).sum(
                        dim=1).count_nonzero(
                        1).unsqueeze(0)
                    ...
                    for l0_norm_idx in range(self.n_l0_norms):
                        output = self.model.forward(x + batch_l0_norms_adv_perts[l0_norm_idx])
                        false_batch = ~y.eq(output.max(dim=1)[1]).detach().to(orig_device)
                        non_robust_indices = batch_indices[false_batch]
                        l0_norms_robust_flags[l0_norm_idx, non_robust_indices] = False
            torch.cuda.synchronize()
            adv_compute_times = [s.elapsed_time(e) for s, e in zip(start_events, end_events)]
            adv_compute_time = np.sum(adv_compute_times)
            adv_compute_time_std = np.std([time * n_batches for time in adv_compute_times])
        ...
        with torch.no_grad():
            ...
            if self.verbose:
                ...
                print("Attack total runtime: " + str(adv_compute_time) + " s" )
                print("Attack total runtime std over batches: " + str(adv_compute_time_std) + " s" )
            ...
            return init_accuracy, x_adv, y_adv, l0_norms_adv_x, \
                l0_norms_robust_accuracy, l0_norms_acc_steps, l0_norms_avg_loss_steps, \
                l0_norms_perts_info, adv_compute_time, adv_compute_time_std
