...
class GFattack:
    ...
    def report_schematics(self):
        print("The attack will compute an adversarial perturbation then attempt to reduce it's L0 norm")
    ...
    def perturb(self, data, target, targeted=False):
        ...
        pert_l0_norm = adv_noise.abs().view(1, 3, -1).sum(dim=1).count_nonzero(
            1).unsqueeze(0)

        return adv_noise, pert_l0_norm, None, None
