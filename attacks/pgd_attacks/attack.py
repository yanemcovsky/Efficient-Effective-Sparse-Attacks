...
class Attack:
    def __init__(self, model, criterion, misc_args, pgd_args, dropout_args=None):
        ...
        self.a_abs = torch.tensor(self.alpha).abs()
        ...
        self.mask_shape_flat = None
        self.mask_zeros_flat = None
        self.mask_ones_flat = None
    ...
    def compute_dpo_args(self, dropout_args):
        ...
        elif dpo_dist_str == "continuous_bernoulli":
    ...
    def project(self, perturbation, mask):
        if self.norm == 'Linf':
    ...
    def step(self, pert, grad, mask):
        grad = self.normalize_grad(grad)
        pert += self.multiplier * grad
        return self.project(pert, mask)
