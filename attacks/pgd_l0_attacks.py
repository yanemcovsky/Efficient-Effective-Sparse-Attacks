...
from attacks.utils import get_predictions, get_predictions_and_gradients
...
class PGDL0attack():
  ...
  def perturb(self, x_tensor, y_tensor, targeted=False):
    ...
    pert_l0_norm = l0_adv_pert.abs().view(x_tensor.shape[0], self.data_channels, -1).sum(dim=1).count_nonzero(1).unsqueeze(0)
