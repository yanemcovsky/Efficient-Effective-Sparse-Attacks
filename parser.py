...
from attacks.cornersearch_attacks import CSattack
from attacks.pgd_l0_attacks import PGDL0attack as PGDL0
...
def parse_args():
    ...
    parser.add_argument('--imagenet_resize', type=int, default=299, help='imagenet data will be transformed to resize X resize (default: 299)')
    ...
    parser.add_argument('--l0_hist_limits', type=int, nargs='+',
                        default=[0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 299, 512, 1024],
    ...
    # attack args
    # pgd attacks args
    parser.add_argument('--attack', type=str, default='PGDTrim', help='PGDTrim, CS, PGD_L0, GF')
    ...
    # pgd attacks dropout args
    parser.add_argument('--att_dpo_dist', type=str, default='bernoulli', help='distribution for dropout sampling in pgd_attacks, options: none, bernoulli, continuous_bernoulli, gauss')
    # pgd attacks pixel trim args
...
def compute_data_args(args):
    ...
    if args.dataset == "imagenet" and not len(args.model_name):
        args.normalize_data = True
    if args.normalize_data:
        if args.attack != 'GF':
            args.model_transform_input = True
            args.data_RGB_offset = 0
            args.data_RGB_size = 1
            args.data_RGB_start = 0
            args.data_RGB_end = 1
        else:
            args.model_transform_input = False
            args.data_RGB_offset = 1
            args.data_RGB_size = 2
            args.data_RGB_start = -1
            args.data_RGB_end = 1
            std_norms = (0.5, 0.5, 0.5)
            mean_norms = (0.5, 0.5, 0.5)
    ...
    if args.dataset == "imagenet":
        args.labels_str_dict = {0: 'tench, Tinca tinca',
        ...
        args.n_classes = 1000
        if args.imagenet_resize == 224:
            transforms_list = [transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor()]
        else:
            transforms_list = [transforms.Resize(args.imagenet_resize), transforms.CenterCrop(args.imagenet_resize), transforms.ToTensor()]
...
def compute_attack_args(args):
    ...
    if args.att_dpo_dist == "none":
        args.att_dpo_mean = 0
        args.att_dpo_std = 0
        args.att_scale_dpo_mean = False
        args.att_dpo_std_bernoulli = False
        args.att_post_trim_dpo = False
        args.dpo_dis_str = "no_dropout"
    else:
        args.att_dpo_mean = 1
        args.att_dpo_std = 1
        args.att_scale_dpo_mean = True
        args.att_dpo_std_bernoulli = True
        args.att_post_trim_dpo = False
    ...
    if args.att_trim_steps is not None:
        ...
        args.att_trim_str = ("list_" + str(args.att_trim_steps).replace('[', '')
                             .replace(']', '').replace(' ', '').replace(',', '_'))
    else:
        args.att_trim_str = "max_steps_" + str(args.att_max_trim_steps)
    args.att_mask_dist_str = "multinomial_n_samples_" + str(args.att_n_mask_samples)

    attack_dict = {'PGDTrim': PGDTrim, 'CS': CSattack, 'PGD_L0': PGDL0, 'GF': GFattack}
